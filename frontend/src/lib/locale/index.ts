...
function generateOneLocale(loc: string) {
	...
	return [
		//////// elven
		{
			...
			loader: async () => (await import(`./${elvenPath}/components.json`)).default
		},
		{
			locale: loc,
			key: 'elven.plugins',
			loader: async () => (await import(`./${elvenPath}/plugins.json`)).default
		},
		//
		{
			locale: loc,
			key: 'elven.index',
			routes: ['/elven'],
			loader: async () => (await import(`./${elvenPath}/index.json`)).default
		},
		{
			locale: loc,
			key: 'elven.articles',
			routes: ['/elven/articles', '/elven/articles/create', '/blog'],
			loader: async () => (await import(`./${elvenPath}/articles.json`)).default
		},
		{
			locale: loc,
			key: 'elven.files',
			routes: ['/elven/files', '/elven/articles/create'],
			loader: async () => (await import(`./${elvenPath}/files.json`)).default
		},
		{
			locale: loc,
			key: 'elven.auth',
			routes: ['/elven/login', '/elven/logout'],
			loader: async () => (await import(`./${elvenPath}/auth.json`)).default
		},
		{
			locale: loc,
			key: 'elven.settings',
			routes: ['/elven/settings'],
			loader: async () => (await import(`./${elvenPath}/settings.json`)).default
		}
	];
}

...
class AuthMaster {
  ...
  public static async login(username: string, password: string, adminLogin: boolean, request: RequestContract): Promise<string> {
    ...
    if (!user) {
      const error = await EL_Errors.privateError('USER_NOT_FOUND', 'Пользователь не найден.')
      return Promise.reject(error)
    }
    if (adminLogin && user.role !== 'admin') {
      const error = await EL_Errors.privateError('USER_NOT_ADMIN', 'Пользователь не админ.')
      return Promise.reject(error)
    }
    const isPassword = await bcrypt.compare(password, user.password)
    if (!isPassword) {
      const error = EL_Errors.privateError('WRONG_PASSWORD', 'Неверный пароль.')
      return Promise.reject(error)
    }
    const length = await EL_Random.randInt(8, 24)
    const trash = await EL_Random.randString(length, 'hex')
    ...
    try {
      ...
      const err = await EL_Errors.privateError('TOKEN_SAVING_ERROR', 'Ошибка при сохранении токена.')
  ...
  public static async getAuthHeader(request: RequestContract): Promise<string> {
    ...
    const error = await EL_Errors.privateError('NO_AUTH_HEADER', 'Authorization header не предоставлен.')
    return Promise.reject(error)
  }
  ...
  public static async getUserAndTokenInstances(token: string) {
    ...
    try {
      ...
      const err = await EL_Errors.privateError('TOKEN_DAMAGED', 'Токен поврежден.')
      return Promise.reject(err)
    }
    if (!decrypted.id) {
      // если в дешифрованном токене нет ID
      const err = await EL_Errors.privateError('TOKEN_MISSING_DATA', 'В токене нет необходимых данных.')
      return Promise.reject(err)
    }
    // получаем сущность токена по поиску его байтов в базе
    const tokenInstance = await Token.findBy('token', decryptedString)
    if (!tokenInstance) {
      const err = await EL_Errors.privateError('TOKEN_NOT_FOUND', 'Токен не найден.')
      return Promise.reject(err)
    }
    ...
    if (tokenUserID !== userID) {
      ...
      const err = await EL_Errors.privateError('TOKEN_STRANGE_OWNER', 'Токен не принадлежит этому пользователю.')
      return Promise.reject(err)
    }
    // ищем владельца токена
    const user = await User.find(userID)
    if (!user) {
      const err = await EL_Errors.privateError('TOKEN_OWNER_NOT_FOUND', 'Владелец токена не найден.')
      return Promise.reject(err)
    }

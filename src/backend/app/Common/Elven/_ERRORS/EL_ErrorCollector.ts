import {
  ...
  E_NOTFOUND,
  E_UNKNOWN,
  E_VALIDATION_MINMAX, E_VALIDATION_MUSTBE
...
export class EL_ErrorCollector {
  ...
  public addError(error: E_UNKNOWN | E_CUSTOM | E_AUTH_INCORRECT
    | E_AUTH_FORBIDDEN | E_NOTFOUND | E_VALIDATION_MUSTBE | E_VALIDATION_MINMAX): boolean {
    this.errorsArray.push(error)
    return true
  }

  public static singleError(error: E_UNKNOWN | E_CUSTOM | E_AUTH_INCORRECT
    | E_AUTH_FORBIDDEN | E_NOTFOUND | E_VALIDATION_MUSTBE | E_VALIDATION_MINMAX): object {
    return {errors: [error]}
  }

  public getErrors(): { errors: object[] } {
    if (!this.hasErrors()) {
      const unknown = new E_UNKNOWN(['SYSTEM'], 'Auto-generated error. Seems like developer forgot add error. Its sad.')

<script lang="ts">
  ...
  import NetworkAuth from "../../network/network_auth";
  ...
  function makeLogin() {
    NetworkAuth.login(username, password)
      .then(() => {
        push("/");
      })
      .catch(() => {});
  ...
  function onEnter(event: KeyboardEvent) {
    ...
    const byEnterAndValid =
      event.code === "Enter" && username.length > 0 && password.length > 0;
    if (byEnterAndValid) {
      makeLogin();
    }
  }

...
export default class ElvenPlayerWorker {
    ...
    private readonly slidersMargin = 6 // in pixels
    ...
    // when user set something by mouse pressing, like progress slider
    ...
    private computeProgressPreview(pageX: number) {
        ...
        this.progressPreviewTime = clickPosition * this.audioPlayerEL.duration - this.slidersMargin
    ...
    private setVolume(percents: number) {
        ...
        this.volumeSliderBubbleEL.style.left = `calc(${percents}% - ${this.slidersMargin}px)` // -6 = margin
    ...
    private getClickPosition(pageX: number, element: HTMLElement): number {
        return (pageX - element.offsetLeft) / element.offsetWidth
    }

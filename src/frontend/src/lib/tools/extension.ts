...
export function generateFileTypeSelector(
	readable: FileExtensionReadable | FileExtensionReadable[],
	extensions: FileExtensions = FileExtensionsDefault
): FileTypeSelector {
	...
	const select: FileTypeSelector = {
		...
		selectedToString: () => {
			return selectedToString(readable, extensions);
		}
	};
	return select;
}

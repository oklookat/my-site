...
func (ec *errorCollector) AddEAuthIncorrect(issuers []string, message string) {
	_error := EAuthIncorrect{
		EError{StatusCode: 403, ErrorCode: "E_AUTH_INCORRECT", Issuers: issuers, Message: message},
	}
	ec.errorsArray = append(ec.errorsArray, _error)
}

// AddEAuthForbidden like 'only admins'
func (ec *errorCollector) AddEAuthForbidden(issuers []string, message string) {
	_error := EAuthForbidden{
		EError{StatusCode: 403, ErrorCode: "E_AUTH_FORBIDDEN", Issuers: issuers, Message: message},
	}
	ec.errorsArray = append(ec.errorsArray, _error)
}

// AddENotFound like 'article not found'
func (ec *errorCollector) AddENotFound(issuers []string, message string) {
	_error := ENotFound{
		EError{StatusCode: 404, ErrorCode: "E_NOTFOUND", Issuers: issuers, Message: message},
	}
	ec.errorsArray = append(ec.errorsArray, _error)
}

// AddEValidationMustBe like 'is_published must be true or false'
func (ec *errorCollector) AddEValidationMustBe(issuers []string, message string, available []string) {
	_error := EValidationMustBe{
		EError{StatusCode: 400, ErrorCode: "E_VALIDATION_MUSTBE", Issuers: issuers, Message: message},
		available,
	}
	ec.errorsArray = append(ec.errorsArray, _error)
}
...
func (ec *errorCollector) AddEValidationMinMax(issuers []string, min int, max int) {
	_error := EValidationMinMax{
		EError{StatusCode: 400, ErrorCode: "E_VALIDATION_MINMAX", Issuers: issuers, Message: "many or not enough characters"},
		min, max,
	}
	ec.errorsArray = append(ec.errorsArray, _error)
}
...
func (ec *errorCollector) AddEValidationEmpty(issuers []string, message string) {
	_error := EValidationEmpty{
		EError{StatusCode: 400, ErrorCode: "E_VALIDATION_EMPTY", Issuers: issuers, Message: message},
	}
	ec.errorsArray = append(ec.errorsArray, _error)
}

// AddEValidationAllowed like 'allowed only numbers'
func (ec *errorCollector) AddEValidationAllowed(issuers []string, message string, allowed []string) {
	_error := EValidationAllowed{
		EError{StatusCode: 400, ErrorCode: "E_VALIDATION_ALLOWED", Issuers: issuers, Message: message},
		allowed,
	}
	ec.errorsArray = append(ec.errorsArray, _error)
}

...
// making things before something.
type Middleware func(next http.Handler) http.Handler

// internal representation of Middleware.
type middleware struct {
	// all middleware executed = true, one of middleware send response = false.
	executed bool
	// middlewares here.
	chain http.Handler
}

// make one big Middleware from middlewares. At the end, call ServeHTTP.
func (m *middleware) add(middlewares []Middleware) {
	m.chain = m.makeChain(middlewares, m)
}

// calls when all middlewares executed.
func (m *middleware) ServeHTTP(response http.ResponseWriter, request *http.Request) {
	m.executed = true
}

// make one big middleware from middlewares. At the end, call next.
